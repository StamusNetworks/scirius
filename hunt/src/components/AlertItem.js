...
export default class AlertItem extends React.Component {
  ...
  render() {
    ...
    const ipParams = (
      <div>
        {' '}
        {data.src_ip} <span className="glyphicon glyphicon-arrow-right"></span> {data.dest_ip}
      </div>
    );
    let sourceNetwork;
    let targetNetwork;
    ...
    const addInfo = [
      <ListViewInfoItem key="timestamp">
        <p>{moment(data.timestamp).format('YYYY-MM-DD, hh:mm:ss a')}</p>
      </ListViewInfoItem>,
      <ListViewInfoItem key="app_proto">
        <p>Proto: {data.app_proto}</p>
      </ListViewInfoItem>,
      <ListViewInfoItem key="host">
        <p>Probe: {data.host}</p>
      </ListViewInfoItem>,
      <ListViewInfoItem key="category">
    ...
    return (
      <ListViewItem
        ...
        description={
          <span data-toggle="tooltip" title={data.alert.signature}>
        ...
        <Tabs id="alert-tabs">
          <Tab eventKey="alert" title="Synthetic view">
                ...
                <div className="card-pf">
                  ...
                  <div className="card-pf-body">
                    <dl className="dl-horizontal">
                      ...
                      {data.geoip && data.geoip.provider && data.geoip.provider.autonomous_system_number && (
                        <ErrorHandler>
                          <EventField
                      ...
                      {data.geoip && data.geoip.provider && data.geoip.provider.autonomous_system_organization && (
                        <ErrorHandler>
                          <EventField
            ...
            <Row>
                    ...
                    <div className="card-pf-body">
                      <dl className="dl-horizontal">
                        ...
                        {data.tls.ja3 &&
                          ...
                          data.tls.ja3.agent.map((agent, idx) => (
              ...
              {data.dns !== undefined && dnsQuery !== undefined && (
                <Col sm={4}>
                  <div className="card-pf">
                    <div className="card-pf-heading">
                      <h5>DNS</h5>
                    ...
                    <div className="card-pf-body">
                      <dl className="dl-horizontal">
                        {dnsQuery.rrname !== undefined && (
                          <ErrorHandler>
                            <EventField field_name="Queried Name" field="dns.query.rrname" value={dnsQuery.rrname} addFilter={this.addFilter} />
                          </ErrorHandler>
                        )}
                        {dnsQuery.rrtype !== undefined && (
                          <ErrorHandler>
                            <EventField field_name="Queried Type" field="dns.query.rrtype" value={dnsQuery.rrtype} addFilter={this.addFilter} />
                          </ErrorHandler>
                        )}
                      </dl>
                    </div>
                      ...
                      <dl className="dl-horizontal">
                        ...
                        {data['ftp-data'].filename !== undefined && (
                          <ErrorHandler>
                            <EventField
                              field_name="Filename"
                              field="ftp-data.filename"
                              value={data['ftp-data'].filename}
                              addFilter={this.addFilter}
                            />
                          </ErrorHandler>
                        )}
                  ...
                  <div className="card-pf">
                    ...
                    <div className="card-pf-body">
                      <dl className="dl-horizontal">
                        ...
                        <ErrorHandler>
                          <EventField
                            field_name="Flow start"
                            field="flow.start"
                            value={data.flow.start}
                            addFilter={this.addFilter}
                            magnifiers={false}
                          />
                        </ErrorHandler>
                        <ErrorHandler>
                          <EventField
                            field_name="Pkts to server"
                            field="flow.pkts_toserver"
                            value={data.flow.pkts_toserver}
                            addFilter={this.addFilter}
                          />
                        </ErrorHandler>
                        <ErrorHandler>
                        ...
                        <ErrorHandler>
                          <EventField
                            field_name="Pkts to client"
                            field="flow.pkts_toclient"
                            value={data.flow.pkts_toclient}
                            addFilter={this.addFilter}
                          />
                        </ErrorHandler>
                        <ErrorHandler>
          ...
          {showTabs && (
            <Tab eventKey="json-alert" title="JSON View">
              <ReactJson
                name={false}
                src={data}
          ...
          {showTabs && (
            <Tab eventKey="json-related" title={this.formatString('Related events {0}', this.nbEvents(events))}>
