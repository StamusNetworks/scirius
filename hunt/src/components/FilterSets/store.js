...
export const filterSetsReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case DELETE_FILTER_SET:
            ...
            .set('filterSetsLoading', true)
            .set('filterSetsStatus', false)
            .set('filterSetsMessage', 'deleting filter set...');
        ...
        case DELETE_FILTER_SET_SUCCESS:
            ...
            .setIn(['filterSets', action.filterSetType], fromJS(state.getIn(['filterSets', action.filterSetType]).toJS().filter((f) => f.id !== action.filterSetIdx)));
...
export const makeSelectGlobalFilterSets = () => createSelector(makeSelectFiltersSetsStore, (filterSetsState) => filterSetsState.getIn(['filterSets', 'global']).toJS());
export const makeSelectPrivateFilterSets = () => createSelector(makeSelectFiltersSetsStore, (filterSetsState) => filterSetsState.getIn(['filterSets', 'private']).toJS());
export const makeSelectStaticFilterSets = () => createSelector(makeSelectFiltersSetsStore, (filterSetsState) => filterSetsState.getIn(['filterSets', 'static']).toJS());
export const makeSelectFilterSetsLoading = () => createSelector(makeSelectFiltersSetsStore, (filterSetsState) => filterSetsState.get('filterSetsLoading'));
